...
alarm timer from 0m, 0s to 120m, 59s.
...
However, at any time you can **tap** the Pause Count **⟲** button to restart the pause
...
* **Off**: Resets the pause timer without count in. To resume play, **tap** the play
  button.

## Breaks

The pause timer may be augmented with alert messaging and an enforced break. This option
is turned off by default. Use the [Preferences](/docs/settings) setting **Customize
alerts** to enable it.

When turned on, the **Set Time** dialog box for Pause includes a **Custom Alert** check
box.
...
The Force Pause countdown timer disables closing the dialog box as a means to enforce a
break in practice. This countdown can be overridden manually.

Closing the Alert Message dialog box does no restart play. That is done manually with the
Pause Count **⟲** button.
...
- A **Pause** set to **0:00** switches itself off. A countdown with no time to run is not
  a countdown.
- An **Alarm** set to **0:00** stays never sounds. It is stored as an
